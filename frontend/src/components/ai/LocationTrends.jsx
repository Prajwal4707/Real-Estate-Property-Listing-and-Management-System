...
const LocationTrends = ({ locations }) => {
  ...
  if (
    !processedLocations ||
    !Array.isArray(processedLocations) ||
  ...
  const validRentalYields = processedLocations.filter(
    (loc) => loc.rental_yield > 0
  );
  const validAppreciations = processedLocations.filter(
    (loc) => loc.percent_increase > 0
  );

  const bestRentalYield =
    validRentalYields.length > 0
      ? validRentalYields.sort((a, b) => b.rental_yield - a.rental_yield)[0]
      : processedLocations[0];

  const bestAppreciation =
    validAppreciations.length > 0
      ? validAppreciations.sort(
          (a, b) => b.percent_increase - a.percent_increase
        )[0]
      : processedLocations[0];
                    ...
                    <div>
                      ...
                      <span className="font-medium">
                        {location.price_per_sqft
                          ? `₹${location.price_per_sqft.toLocaleString()}`
                          : "N/A"}
                      </span>
                    </div>
                    <div>
              ...
              <table className="min-w-full bg-white">
                <thead>
                  <tr className="bg-gray-50 border-b">
                    <th className="py-3 px-4 text-left font-medium text-gray-700 rounded-tl-lg">
                      Location
                    </th>
                    ...
                    <th className="py-3 px-4 text-left font-medium text-gray-700 rounded-tr-lg">
                      Rental Yield (%)
                    </th>
                  </tr>
                </thead>
              ...
              <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-100 mb-6">
                <div className="flex items-start">
                  ...
                  <p className="text-yellow-700">
                    Some data is missing or incomplete. The insights shown may
                    be limited.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-lg border border-blue-200">
                ...
                <div className="ml-7">
                  ...
                  <div className="text-blue-700">
                    {bestRentalYield?.rental_yield != null
                      ? `${bestRentalYield.rental_yield}% annual return`
                      : "Data not available"}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-green-50 to-green-100 p-4 rounded-lg border border-green-200">
                ...
                <div className="ml-7">
                  ...
                  <div className="text-green-700">
                    {bestAppreciation?.percent_increase != null
                      ? `${bestAppreciation.percent_increase}% annual growth`
                      : "Data not available"}
                  </div>
                </div>
              </div>
              ...
              <ul className="space-y-4 sm:space-y-3 text-gray-700 text-sm sm:text-base">
                {bestRentalYield?.rental_yield > 0 && (
                  ...
                  >
                    ...
                    <span className="break-words">
                      <strong>{bestRentalYield.location}</strong> offers the
                      highest rental yield at {bestRentalYield.rental_yield}%,
                ...
                {bestAppreciation?.percent_increase > 0 && (
                  ...
                  >
                    <span className="inline-block w-1.5 h-1.5 rounded-full bg-blue-500 mt-2 mr-2 flex-shrink-0"></span>
                    <span className="break-words">
                      <strong>{bestAppreciation.location}</strong> shows the
                      strongest appreciation at{" "}
                      {bestAppreciation.percent_increase}%, suggesting good
                ...
                >
                  <span className="inline-block w-1.5 h-1.5 rounded-full bg-blue-500 mt-2 mr-2 flex-shrink-0"></span>
                  <span className="break-words">
                    Areas with rental yields above 4% and appreciation above 8%
                    offer balanced investment opportunities for both income and
                    growth.
                  </span>
                </motion.li>
                ...
                {processedLocations.some(
                  (loc) =>
                    loc.rental_yield == null || loc.percent_increase == null
                ) && (
                  <motion.li
                    initial={{ opacity: 0, x: -10 }}
                    animate={{ opacity: 1, x: 0 }}
                  ...
                  >
                    ...
                    <span>
                      Some locations have missing data. Consider this when
                      making investment decisions or consult with a local real
                      estate expert for more complete information.
                    </span>
                  </motion.li>
                )}
              </ul>
...
LocationTrends.propTypes = {
  locations: PropTypes.arrayOf(
    PropTypes.shape({
      ...
      percent_increase: PropTypes.oneOfType([
        PropTypes.string,
        PropTypes.number,
      ]),
